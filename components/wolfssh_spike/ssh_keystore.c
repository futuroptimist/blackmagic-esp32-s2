    ...
    uint8_t* der_out, size_t der_out_cap, size_t* der_len_out)
{
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        /* No stored key after successful partition initialization means
         * genuine first boot or an explicit trusted factory reset. nvs_init()
         * preserves unexpected recovery errors and prevents SSH startup. */
        size_t der_len = 0;
