...
- The HTTP server currently has no authentication. It serves the landing page
  ...
  passwords and reports only whether each is configured, while
  `/api/v1/wifi/set_credentials` requires explicit keep, replace, or clear
  actions for each password. The configuration-writing route,
  `/api/v1/system/reboot` reboots the board, and `/api/v1/uart/websocket` is a
  bidirectional UART bridge; all remain unauthenticated (`main/network-http.c`,
...
their current unauthenticated security properties. The coordinated UI/API now
redacts stored passwords and distinguishes keeping, replacing, and clearing
them. The remaining prerequisite for SSH rollout is to authenticate, disable,
or equivalently restrict state-changing HTTP routes and the UART WebSocket;
this credential-confidentiality fix does not authenticate the HTTP management
plane.
