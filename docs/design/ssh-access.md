...
- The HTTP server currently has no authentication. It serves the landing page
  ...
  particular, `/api/v1/wifi/get_credentials` redacts AP and station passwords
  and reports only whether each is configured. Password writes through
  `/api/v1/wifi/set_credentials` explicitly distinguish keeping, replacing, and
  clearing each password. State-changing routes such as that endpoint and
...
Preserving `/`, `/config`, and HTTP API functionality does not make the HTTP
management plane authenticated. The coordinated UI and API now redact stored
passwords and distinguish leaving a stored password unchanged from explicitly
replacing or clearing it. The remaining prerequisite for SSH rollout is to
authenticate, disable, or equivalently restrict state-changing routes; this
credential-confidentiality hotfix does not secure the broader HTTP management
plane.
