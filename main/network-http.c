...
static esp_err_t wifi_set_credentials_handler(httpd_req_t* req) {
    ...
    int received = 0;
    bool ap_pass_replace = false;
    bool ap_pass_clear = false;
    bool sta_pass_replace = false;
    bool sta_pass_clear = false;
...
#define GET_REQUIRED_STRING(field, target)                                      \
    do {                                                                         \
        ...
        if(!cJSON_IsString(item)) {                                               \
            error_text = JSON_ERROR("missing or invalid [" field "] field");  \
            goto err_fail;                                                       \
        }                                                                        \
...
#define VALIDATE_PASS_ACTION(action, pass_item, pass, replace, clear, name)          \
    do {                                                                             \
        if(strcmp((action)->valuestring, "replace") == 0) {                         \
            if(!cJSON_IsString(pass_item)) {                                         \
                error_text = JSON_ERROR("replacement requires [" name "] string"); \
                goto err_fail;                                                       \
            }                                                                        \
            ...
            if(pass_item != NULL) {                                                  \
                error_text = JSON_ERROR("[" name "] must be absent for clear");   \
                goto err_fail;                                                       \
            }                                                                        \
            clear = true;                                                            \
        } else if(strcmp((action)->valuestring, "keep") == 0) {                    \
            if(pass_item != NULL) {                                                  \
                error_text = JSON_ERROR("[" name "] must be absent for keep");    \
                goto err_fail;                                                       \
            }                                                                        \
        } else {                                                                     \
    ...
    VALIDATE_PASS_ACTION(
        ap_action, ap_pass_item, ap_pass, ap_pass_replace, ap_pass_clear, "ap_pass");
    VALIDATE_PASS_ACTION(
        sta_action, sta_pass_item, sta_pass, sta_pass_replace, sta_pass_clear, "sta_pass");
...
#undef GET_REQUIRED_STRING
    ...
    if(mstring_size(ap_ssid) == 0 || mstring_size(ap_ssid) > 32 ||
       ...
       (ap_pass_replace && (mstring_size(ap_pass) < 8 || mstring_size(ap_pass) > 64)) ||
       (sta_pass_replace && (mstring_size(sta_pass) < 8 || mstring_size(sta_pass) > 64))) {
    ...
    if((ap_pass_replace || ap_pass_clear) && nvs_config_set_ap_pass(ap_pass) != ESP_OK) {
    ...
    if((sta_pass_replace || sta_pass_clear) && nvs_config_set_sta_pass(sta_pass) != ESP_OK) {
